...
Parameters = struct('R',100,'Vm',10,'Box',1000,'NRef',0);
...
Xinferred = Matricize('../BGFS_optimization/Inputs/CSV4John.csv', N, d, tm+1);
Xreal = Matricize('../BGFS_optimization/Inputs/CSV4.csv', N, d, tm+1);
...
name = sprintf('ViolationsN%dT%dR%d.jpg',N,tm,R);
saveas(fig1,name)
...
name = sprintf('ObjectiveN%dT%dR%d.jpg',N,tm,R);
saveas(fig2,name)
...
name = sprintf('MSEN%dT%dR%d.jpg',N,tm,R);
saveas(fig3,name)
...
ylabel('Error in %')
hold off;
name = sprintf('LinkErrorsN%dT%dR%d.jpg',N,tm,R);
saveas(fig4,name)
...
video = true;
if(video)
    name = sprintf('Locations%dT%dR%d.avi',N,tm,R);
    profile.FrameRate = 1;
