...
Res = load('./Results/CSV42012111.mat');
Res = struct2cell(Res);
Res = Res{1};
Xreal = Res{1};
Xp = Xreal(:,:,1);
...
% Out of range violation means that the distance between nodes is smaller
% than R while the contact trace shows no link between the nodes (i.e. 
% extra link)
ORV = zeros(1,tm);
% In range violation means that the distance between nodes is larger
% than R while the contact trace shows a link between the nodes (i.e. 
% missing link)
IRV = zeros(1,tm);
% Spped violation means that the nodes has moved a distance larger than
% V_max X t in a timestep
SV = zeros(1,tm);
% Total violations is simply the sum of the number of times each of the
% above constraints has been violated
TV = zeros(1,tm);
...
title('Out of Range Violations(Extra Links)')
xlabel('Time')
ylabel('Violations %')
...
title('In Range Violations(Missing Links)')
xlabel('Time')
ylabel('Violations %')
...
name = sprintf('./Figures/%sViolationsN%dT%dR%d.jpg',Day,N,tm,R);
saveas(fig1,name)
...
name = sprintf('./Figures/%sObjectiveN%dT%dR%d.jpg',Day,N,tm,R);
saveas(fig2,name)
...
name = sprintf('./Figures/%sMSEN%dT%dR%d.eps',Day,N,tm,R);
saveas(fig3,name)
...
fig4 = figure(4);
hold on;
MissingLinks = smooth(1:tm,MissingLinks,21);
ExtraLinks = smooth(1:tm,ExtraLinks,21);
LinkErrors = smooth(1:tm,LinkErrors,21);
% r-- b.- k-
plot(1:tm,MissingLinks,'r');
plot(1:tm,ExtraLinks,'b');
plot(1:tm,LinkErrors,'k')
legend('Missing Links','Extra Links','Total Link Errors')
title('Link Prediction Errors in Connectivity Graph')
xlabel('Time')
ylabel('Error in %')
hold off;
name = sprintf('./Figures/%sLinkErrorsN%dT%dR%d.eps',Day,N,tm,R);
saveas(fig4,name)
...
video = true;
if(video)
    name = sprintf('./Figures/%sLocations%dT%dR%d.avi',Day,N,tm,R);
    profile.FrameRate = 1;
